...
## Current Snapshot
- `widget/v1.js` endpoint is available and serves the SlipAI script through the stable auto-update URL.
...
- Lovable/low-code test installs should use either the `data-*` one-script snippet or the now-supported `window.slipai("init", ...)` queue snippet, pointing to `/widget/v1.js` without a version query for production.
