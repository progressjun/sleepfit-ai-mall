...
2. After Vercel is connected, set `SLIPAI_HEALTH_BASE_URL` to the public URL and run `node scripts/slipai-health-check.mjs`.
3. Keep the 3-hour Codex maintenance automation active on the `SlipAI` branch and review any proposed safe fixes before merging.
